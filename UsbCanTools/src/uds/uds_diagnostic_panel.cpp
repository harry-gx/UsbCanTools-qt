...
QList<ServiceTemplateEntry> diagnosticServiceTemplates()
{
    ...
    t << ServiceTemplateEntry{QStringLiteral("10 - 会话控制"), QStringLiteral("会话控制"), QStringLiteral("RAW"), QStringLiteral("1001"), 3000, 0, true, QStringLiteral("50")};
    t << ServiceTemplateEntry{QStringLiteral("11 - ECUReset"), QStringLiteral("ECUReset"), QStringLiteral("RAW"), QStringLiteral("1101"), 3000, 0, true, QStringLiteral("51")};
    ...
    t << ServiceTemplateEntry{QStringLiteral("22 - 读DID"), QStringLiteral("读DID"), QStringLiteral("RAW"), QStringLiteral("22F190"), 3000, 0, false, QStringLiteral("62")};
    t << ServiceTemplateEntry{QStringLiteral("2E - 写DID"), QStringLiteral("写DID"), QStringLiteral("RAW"), QStringLiteral("2EF1900001"), 3000, 0, false, QStringLiteral("6E")};
    t << ServiceTemplateEntry{QStringLiteral("19 - 读DTC信息"), QStringLiteral("读DTC信息"), QStringLiteral("RAW"), QStringLiteral("1902FF"), 3000, 0, false, QStringLiteral("59")};
    t << ServiceTemplateEntry{QStringLiteral("14 - 清除DTC"), QStringLiteral("清除DTC"), QStringLiteral("RAW"), QStringLiteral("14FFFFFF"), 3000, 0, false, QStringLiteral("54")};
    t << ServiceTemplateEntry{QStringLiteral("27 - SecurityAccess请求Seed"), QStringLiteral("请求安全Seed"), QStringLiteral("RAW"), QStringLiteral("2701"), 3000, 1, false, QStringLiteral("67")};
    return t;
}
...
void UdsDiagnosticWidget::buildUi()
{
    ...
    auto *cfgBox = new QGroupBox(QStringLiteral("UDS诊断参数"), this);
    ...
    cfgForm->addRow(QStringLiteral("执行循环次数"), m_loopCountSpin);
    ...
    auto *serviceBox = new QGroupBox(QStringLiteral("标准UDS服务模板"), this);
    ...
    auto *loadDefaultBtn = new QPushButton(QStringLiteral("载入默认流程"), this);
    auto *importBtn = new QPushButton(QStringLiteral("导入流程"), this);
    auto *exportBtn = new QPushButton(QStringLiteral("导出流程"), this);
    ...
    connect(loadDefaultBtn, &QPushButton::clicked, this, &UdsDiagnosticWidget::onFlowLoadDefault);
    ...
    flowBtnRow->addWidget(loadDefaultBtn);
    ...
    auto *flowBox = new QGroupBox(QStringLiteral("诊断流程编排(可编辑)"), this);
    ...
    auto *execFlowBox = new QGroupBox(QStringLiteral("执行器流程(来自编辑器)"), this);
                                     ...
                                     const QString &expectedSidHex)
{
    ...
    m_flowTable->setItem(row, 3, new QTableWidgetItem(requestHex));
    ...
    m_flowTable->setItem(row, 7, new QTableWidgetItem(expectedSidHex));
}
...
bool UdsDiagnosticWidget::buildFlowFromTable(QTableWidget *table, QList<UdsFlashStep> *steps, QString *err) const
{
    ...
    for (int row = 0; row < table->rowCount(); ++row) {
        ...
        if (type == QStringLiteral("RAW")) {
            ...
            if (err)
                *err = QStringLiteral("第%1行 类型仅支持 RAW/AUTO_10/AUTO_11").arg(row + 1);
            return false;
        }
        ...
        if (step.checkPositiveSid) {
            quint32 sidVal = 0;
            if (!parseHexU32(sidHex, &sidVal) || sidVal > 0xFFu) {
                if (err)
...
void UdsDiagnosticWidget::onFlowImport()
{
    ...
    if (!f.open(QIODevice::ReadOnly)) {
        QMessageBox::warning(this, QStringLiteral("UDS诊断"), QStringLiteral("流程导入失败: 无法读取文件"));
        return;
    }
    ...
    if (pe.error != QJsonParseError::NoError || !doc.isObject()) {
        QMessageBox::warning(this, QStringLiteral("UDS诊断"), QStringLiteral("流程导入失败: JSON格式不正确"));
        return;
    }

    const QJsonArray arr = doc.object().value(QStringLiteral("steps")).toArray();
    if (arr.isEmpty()) {
        QMessageBox::warning(this, QStringLiteral("UDS诊断"), QStringLiteral("流程导入失败: steps为空"));
        return;
    }
...
void UdsDiagnosticWidget::onFlowExport()
{
    const QString path = QFileDialog::getSaveFileName(this,
                                                      QStringLiteral("导出流程"),
                                                      QStringLiteral("uds_diag_flow.json"),
                                                      QStringLiteral("UDS Flow (*.json)"));
    if (path.isEmpty())
        return;
    ...
    if (!f.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        QMessageBox::warning(this, QStringLiteral("UDS诊断"), QStringLiteral("流程导出失败: 无法写入文件"));
        return;
    }
